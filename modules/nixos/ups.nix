...
let
  cfg = config.introdus.services.ups;
...
in
{
  ...
  config = lib.mkIf (useUps) {
    ...
    # we assert that mail-delivery is enabled at the end of this module
    introdus.mail-delivery.extraUsers =
      config.introdus.mail-delivery.extraUsers ++ config.power.ups.upsmon.user;
    ...
    assertions = lib.optional cfg.server.enable [
      {
      ...
      {
        assertion = config.introdus.mail-delivery.enable;
        message = "introdus.ups requires that introdus.mail-delivery be enabled";
      }
    ];
  };
}
