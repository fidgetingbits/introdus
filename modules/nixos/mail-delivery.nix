# Options to determine how msmtp or a postfix-relay will be used to deliver email notifications
{ config, lib, ... }:
let
  cfg = config.introdus.mail-delivery;
  secret = "passwords/${if cfg.useRelay then "postfix-relay" else "msmtp"}";
in
{
  options.introdus.mail-delivery = {
    enable = lib.mkEnableOption "Enable mail-delivery";
    ...
    user = lib.mkOption {
      ...
      type = lib.types.str;
      description = "Main user added to the group with mail delivery password access";
    };
