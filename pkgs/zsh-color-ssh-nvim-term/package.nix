...
let
  ...
  scriptFile = pkgs.writeShellScriptBin "${pname}.plugin.zsh" ''
        ...
        if [[ "$1" =~ "ssh" ]] && [ -n "$NVIM" ]; then
            ${pkgs.introdus.neovim-python-scripts}/bin/neovim-change-bg-color black
        fi
    ...
    function unset_nvim_term_ssh_color() {
        if [ -n "''${NVIM_TERM_SSH_COLOR_SET}" ]; then
          ${pkgs.introdus.neovim-python-scripts}/bin/neovim-change-bg-color None
...
stdenv.mkDerivation {
  ...
  dontBuild = true;
  dontUnpack = true;
  runtimeInputs = [ pkgs.introdus.neovim-python-scripts ];
  installPhase = ''
